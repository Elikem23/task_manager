...
def show_edit_task_frame(task,app):
    ...
    # Add an entry widget and show the task title in it
        ...
        entry.grid( row=1, column=0, columnspan=2, sticky="nsew", pady=(0,10) )
    ...
    # Add a button with text Back/ Cancel for the remove
        cancel_btn=tk.Button(
        master=frame, text="Back",
...
def show_all_tasks_frame(app):
    ...
    for task in tasks:
        ...
        btn=tk.Button(
            master=frame, 
            text="Delete",
            command=partial(show_add_task_frame, task["_id"],app))
